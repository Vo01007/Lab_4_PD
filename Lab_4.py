...
annotation_folder = 'D:\\LABS\\Lab_PD\\Lab 4'
...
def create_annotation_file(source_folder, class_name):
    with open('annotations.csv', 'w', newline='') as csv_file:
        ...
        writer.writerow(['D:\\LABS\\Lab_PD\\Lab 3\\datasetL3', '..\\Lab 3\\datasetL3', 'polar_bear'])
...
class DatasetApp(QtWidgets.QWidget):
    ...
    def create_annotation(self):
        if self.folderpath:
            create_annotation_file(self.folderpath, 'polar_bear')
            create_annotation_file(self.folderpath, 'brown_baer')
            pass

    def get_next_class1(self):
        if self.folderpath:
            next_instance = get_next_instance('polar_bear', self.folderpath)
            if next_instance:
    ...
    def get_next_class2(self):
        if self.folderpath:
            next_instance = get_next_instance('brown_bear', self.folderpath)
            if next_instance:
                pixmap = QtGui.QPixmap(next_instance)
                self.image_label.setPixmap(pixmap)
